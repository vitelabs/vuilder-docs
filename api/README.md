...
:::tip Transaction IS Account Block
...
* `hex` and `base64` is strings in hex or base64 format 
* For Numeric types such as `uint64`, `float` and `bigint`, when they are tailed with `string` type, means they should be passed as strings
...
|  Name  | Type | Description |
|:------------:|:-----------:|:-----:|
| tokenInfo | [TokenInfo](#tokeninfo) | Token's information |
...
| tokenInfo | [TokenInfo](#tokeninfo) | Token transferred |
